...
const MemoizedQuestionsTable = memo(
  ({ rows, loading, onHistoryButtonClick, onEditButtonClick }) => {
    ...
    return (
      <TableContainer component={Paper} sx={{ marginTop: 2, flexGrow: 1 }}>
            ...
            <TableRow>
              ...
              <TableCell align="left">
                {selectedIds.size ? (
                  <Button
                    variant="outlined"
                    onClick={() => setOpenMutiAssignModal(true)}
                  >
                    Assign
                  </Button>
                ) : null}
          ...
          <TableBody>
            {rows.map(row => (
              ...
              >
                ...
                <TableCell>
                  {row?.tags_list?.map(t => (
                    <Chip sx={{ mr: 2, mt: 1 }} key={t} label={t} />
                  ))}
                </TableCell>
        ...
        <MultiAssignModal
          open={openMutiAssignModal}
          onClose={() => {
            setOpenMutiAssignModal(false);
          }}
          ids={[...selectedIds]}
        />
      </TableContainer>
...
const QuestionsTable = () => {
  const {
    ...
    loading,
    setSelectedQuestion,
    singleQuestion: { setHistoryModalOpen, setEditModalOpen },
  } = useDashboardContext();
